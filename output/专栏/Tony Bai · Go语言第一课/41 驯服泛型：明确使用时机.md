...
Source: https://learn.lianglianglee.com/专栏/Tony Bai · Go语言第一课/41 驯服泛型：明确使用时机.md
...
从上图我们看到，Go编译器为每个底层类型相同的类型生成一份函数代码，像MyInt和int、rune和int32；对于所有指针类型，像上面的\*float64、\_int和\_int32，仅生成一份名为main.f[go.shape.\*uint8\_0]的函数代码。
