...
首先是 ReferenceConfigCache.getCache() 这个静态方法，会在 CACHE\_HOLDER 集合中添加一个 Key 为“\*DEFAULT\*”的 ReferenceConfigCache 对象（使用默认的 KeyGenerator 实现），它将作为默认的 ReferenceConfigCache 对象。
