...
Source: https://learn.lianglianglee.com/专栏/Kafka核心源码解读/25 ReplicaManager（下）：副本管理器是如何管理副本的？.md
...
再之后是执行LeaderAndIsrRequest请求的回调处理逻辑。这里的回调逻辑，实际上只是对Kafka两个内部主题（**consumer\_offsets和**transaction\_state）有用，其他主题一概不适用。所以通常情况下，你可以无视这里的回调逻辑。
