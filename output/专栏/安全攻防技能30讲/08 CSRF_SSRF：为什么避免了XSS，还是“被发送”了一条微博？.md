...
Source: https://learn.lianglianglee.com/专栏/安全攻防技能30讲/08 CSRF_SSRF：为什么避免了XSS，还是“被发送”了一条微博？.md

因收到Google相关通知，网站将会择期关闭。[相关通知内容](https://lumendatabase.org/notices/44265620)

---

# 08 CSRF\_SSRF：为什么避免了XSS，还是“被发送”了一条微博？
...
![](assets/28bef572703d4861bf0252aec620e8fc.jpg)
...
通过前面的学习，我们知道，CSRF是通过自动提交表单的形式来发起攻击的。所以，在前面转账的例子中，黑客可以通过[抓包](https://baike.baidu.com/item/抓包/9929103?fr=aladdin)分析出[http://bank.com/transfer这个接口所需要的参数，从而构造对应的form表单。因此，我们只需要在这个接口中，加入一个黑客无法猜到的参数，就可以有效防止CSRF了。这就是\*\*CSRF](http://bank.com/transfer这个接口所需要的参数，从而构造对应的form表单。因此，我们只需要在这个接口中，加入一个黑客无法猜到的参数，就可以有效防止CSRF了。这就是**CSRF) Token\*\*的工作原理。

它的工作流程，我也总结了一下，如下图所示：

![](assets/13df273e0db947078b85cf7695f2deef.jpg)
...
![](assets/dd29d7e388cc456d922c8cf2255dbb72.jpg)
...
![](assets/75c632f16dbf40cca38d96e7823d050e.jpg)
...
在前面百度搜图的例子中，我们请求的地址是：<https://image.baidu.com/search/detail?objurl=http://s1.sinaimg.cn/picture>[.jpg](https://image.baidu.com/search/detail?objurl=http://s1.sinaimg.cn/picture.jpg的时候，)。因为[http://s1.sinaimg.cn/picture](https://image.baidu.com/search/detail?objurl=http://s1.sinaimg.cn/picture)[.jpg](https://image.baidu.com/search/detail?objurl=http://s1.sinaimg.cn/picture.jpg的时候，)会正常返回一个图片，所以网页会展示出来对应的图片。
...
基于这三种返回逻辑，黑客可以构造一个恶意的请求地址：[https://image.baidu.com/search/detail?objurl=127.0.0.1:3306](https://image.baidu.com/search/detail?objurl=127.0.0.1:3306，)。如果服务器返回“格式错误”，则代表服务端本地的3306端口可用；如果返回“找不到图片”，则代表不可用。我们知道，3306是MySQL对应的端口号，因此，根据这个返回的信息，黑客就能够知道服务端本地是否开启了一个MySQL服务。接下来，黑客只需要不断重复这个过程，尝试不同的IP和端口号，就能够一点一点探测出整个内网的结构。
...
![](assets/11c1afdf141e4eefbb488a9cd59727c9.jpg)
