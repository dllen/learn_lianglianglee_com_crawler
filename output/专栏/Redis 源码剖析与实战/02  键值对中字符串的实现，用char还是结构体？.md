...
Source: https://learn.lianglianglee.com/专栏/Redis 源码剖析与实战/02  键值对中字符串的实现，用char还是结构体？.md
...
![](assets/0f8b2de0dc4b30392e155f0bdca0d003-20221013234815-y0weh7g.jpg)
...
![](assets/7de1b18d30280645c17yydf14afde541-20221013234815-6u6bf9q.jpg)
...
![](assets/772d340bfbfe52de3a66fbb011ac22a3-20221013234815-b6ueuiu.jpg)
...
![](assets/845fd7e227f419b1e6c084cdf051ec3d-20221013234815-954d6s3.jpg)
...
我们可以看到，现有长度 len 和已分配空间 alloc 的数据类型都是 uint8\_t。**uint8\_t 是 8 位无符号整型**，会占用 1 字节的内存空间。当字符串类型是 sdshdr8 时，它能表示的字符数组长度（包括数组最后一位\0）不会超过 256 字节（2 的 8 次方等于 256）。
...
其实这里，**attribute** ((**packed**))的作用就是告诉编译器，在编译 sdshdr8 结构时，不要使用字节对齐的方式，而是**采用紧凑的方式分配内存**。这是因为在默认情况下，编译器会按照 8 字节对齐的方式，给变量分配内存。也就是说，即使一个变量的大小不到 8 个字节，编译器也会给它分配 8 个字节。
